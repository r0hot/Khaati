...
class Post
{
				...
				if(!empty($files['file']['name']))
				{
					...
					$folder = "uploads/" . $userid . "/";
						...
						if(!file_exists($folder))
						{
							...
							file_put_contents($folder . "link.php", "");
						}
	...
	public function like_post($id, $type, $mybook_userid) {
		...
		if (is_array($result) && isset($result[0]['likes'])) {
			...
			$sql = "INSERT INTO likes (type, contentid, likes) VALUES ('$type', '$id', '$likes_string')";
		}
		
		$DB->save($sql);
	
		// Increment or decrement like count
		$like_change = in_array($mybook_userid, $user_ids) ? "-1" : "+1";
		$sql = "UPDATE {$type}s SET likes = likes {$like_change} WHERE {$type}id = '$id' LIMIT 1";
		$DB->save($sql);
