...
// 			$Post = new Post();
// 			// $one_post = $Post->get_one_post($row['parent']); recently done

// 			// if($myid == $one_post['userid']){
// 			// 	return true;
// 			// }
// 			$one_post = $Post->get_one_post($row['parent']);
// 			if ($one_post && is_array($one_post) && isset($one_post['userid']) && $myid == $one_post['userid']) {
// 				return true;
// 			}
...
function i_own_content($row) {
    ...
    // Groups
    if (isset($row['gender']) && $row['type'] == "group" && isset($row['owner']) && $myid == $row['owner']) {
        return true;
    }
    ...
    if (isset($row['postid'])) {
        if ($myid == $row['userid']) {
            ...
            // Groups: posts by moderator or admin
            $DB = new Database();
            if (isset($one_post['owner'])) {
                $groupid = $one_post['owner'];

                $sql = "SELECT * FROM group_members WHERE (role = 'admin' OR role = 'moderator') AND groupid = '$groupid' LIMIT 1";
                $check = $DB->read($sql);
                if (is_array($check)) {
                    return true;
                }
            ...
            if (isset($row['userid'])) {
                $groupid = $row['userid'];
                $sql = "SELECT * FROM users WHERE owner = '$myid' AND userid = '$groupid' LIMIT 1";
                $check = $DB->read($sql);
                if (is_array($check)) {
                    return true;
                }
...
function split_url2()
{
	$url = isset($_GET['url']) ? $_GET['url'] : "link";
